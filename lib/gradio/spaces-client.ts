...
async function getClient(spaceEndpoint: string): Promise<Client> {
  ...
  const client = await Client.connect(spaceEndpoint, {
    hf_token: process.env.HF_TOKEN, // Optional: for rate limit increase
  });
...
/**
 * Clear a specific client from cache (useful for error recovery)
 */
...
export async function generateImage(
  ...
  try {
    ...
    // Clear cache on error for fresh retry
...
function parseImageResult(
  ...
  // Most spaces return image as first element
...
export async function chat(
  ...
  try {
    ...
    // Most chat spaces return [response, history]
...
/**
 * Stream chat responses (for spaces that support it)
 */
...
/**
 * Upscale an image using a Hugging Face Space
 */
export async function upscaleImage(
  spaceId: string,
  imageUrl: string | Blob
): Promise<GenerationResult<{ url: string }>> {
  const startTime = Date.now();
  const space = getSpaceById(spaceId);
  ...
  try {
    ...
    const result = await client.predict(space.apiPath || "/predict", {
      image: imageUrl,
    });

    const data = result.data;
    const outputData = data[0] as { url?: string; path?: string } | string;
    const url = typeof outputData === 'string' 
      ? outputData 
      : outputData?.url || outputData?.path || '';
...
export async function generateMusic(
  prompt: string,
  duration: number = 8
): Promise<GenerationResult<{ audioUrl: string }>> {
  const startTime = Date.now();
  const space = getSpaceById('musicgen');

  if (!space) {
    return {
      success: false,
      error: 'MusicGen space not configured',
      space: 'musicgen',
      duration: Date.now() - startTime,
    };
  }
  ...
  try {
    ...
    const data = result.data;
    const audioData = data[0] as { url?: string; path?: string } | string;
    const audioUrl = typeof audioData === 'string' 
      ? audioData 
      : audioData?.url || audioData?.path || '';
...
/**
 * Check if a space is available/responsive
 */
...
/**
 * Get all available spaces with their status
 */
export async function getAllSpacesStatus(): Promise<
  Array<SpaceConfig & { status: { available: boolean; latency?: number; error?: string } }>
> {
  const { ALL_SPACES } = await import('./spaces-config');
  
  const results = await Promise.all(
    ALL_SPACES.map(async (space) => {
      const status = await checkSpaceStatus(space.id);
      return { ...space, status };
    })
  );
